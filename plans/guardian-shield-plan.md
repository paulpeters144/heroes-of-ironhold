...
The knight's own carried shield sprite is reused — no new image asset is needed. The system resolves it from the `Shield` entity's `StaticImage` child in `new()`, falling back to `images::Knight::Shield1` if the knight's shield is missing.

## Create `sys_guardian_shield.rs`
A new system mirroring `SwordsSkillSystem`'s shape: subscribes to `SkillCastEvent`, ignores kinds other than `SkillIconKind::Shield`, puts the knight into the swipe pose with a `KnightLock`, counts down the 500ms stance, then renders the shield sprite in front of the knight. The shield is pure graphics — drawn by the system's `draw()`, not a store entity, and carries no `AttackRect` or gameplay data. The texture is loaded once in `new()` by resolving the knight's `Shield` entity's `StaticImage` (falling back to `images::Knight::Shield1`) and stored on the struct (never call `Assets` accessors in `update()`/`draw()`).
...
Once the STANCE_SECS (500ms) stance ends the KnightLock is released and the shield becomes active. The shield follows the knight thereafter: each frame `draw_shield` re-reads the knight's current center, so the shield stays in front of the knight. The shield sits `SHIELD_AHEAD` (20px) to the right of the knight's center and is drawn at `GUARDIAN_SHIELD_SIZE` (64px) square — the knight's carried shield sprite at 2x its native 32x32. The shield deactivates after `SHIELD_SECS` of being visible. Alpha fades in over `FADE_IN_SECS` (250ms) at spawn and fades out over `FADE_OUT_SECS` (250ms) before despawn, applied through the draw color's alpha. Nothing else cancels the shield: Swords casts, attacks, and movement leave it up, and a second Guardian Shield cast is ignored while one is already active (only one shield at a time).
...
Tracking does not store a direction: `shield_position` re-resolves the knight's current center each frame, so the shield follows the knight without any cast-time snapshot. The shield sits `SHIELD_AHEAD` to the right of the knight regardless of facing.
...
impl GuardianShieldSystem {
    /// Subscribes to SkillCastEvent and resolves the shield texture from the
    /// knight's Shield entity (falling back to Shield1); returns the ready system.
    pub fn new(store: Rc<EStore>, bus: Rc<EventBus>, assets: &Assets) -> Self { ... }

    /// Resolves the knight's animation ref, sets the swipe pose, adds a
    ...
    /// Returns where the shield should be drawn: the knight's current center plus
    /// SHIELD_AHEAD to the right. Re-read each frame so the shield tracks the knight.
    fn shield_position(&self, knight_center: Vec2) -> Vec2 { ... }

    /// Draws the shield sprite in front of the knight when shield_active is true:
    /// a plain draw_texture_ex of shield_tex at GUARDIAN_SHIELD_SIZE square (the knight's
    /// carried shield sprite at 2x its native 32x32), top-left = shield_position -
    /// GUARDIAN_SHIELD_SIZE/2. Position re-resolves
    /// the knight's current center each frame so the shield tracks the
...
- Whether the sprite should mirror when facing left (currently the shield always draws to the right of the knight) is a visual-taste call — try both in-game.

...
fn each_callback_receives_all_entities_of_type() {
    ...
    store
        .all::<Dwarf>()
        .for_each(|d| names.push(d.name.clone()));
...
fn add_children_dead_entity_error() {
    ...
    let err = store
        .add(dwarf("Thorin", 150), &[ChildSource::Existing(dead_axe)])
        .unwrap_err();
    ...
    let parent = store.first::<Dwarf>().unwrap();
    let err = store
        .add(parent, &[ChildSource::Existing(dead_axe)])
        .unwrap_err();
    assert_eq!(err, PicoError::EntityNotAlive);
}
...
fn entity_ref_debug_contains_id() {
    ...
    assert!(
        debug.contains("0"),
        "debug output should contain entity id: {debug}"
    );
}
...
fn pico_error_display_formats_correctly() {
    assert_eq!(
        PicoError::TypeNotRegistered.to_string(),
        "type not registered"
    );
    assert_eq!(
        PicoError::AlreadyHasParent.to_string(),
